define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        // { field: 'commission', title: __('佣金'), sortable: true },
                        // { field: 'group_number', title: __('团队人数'), operate: false },
                        // { field: 'is_robot', title: __('是否机器人') },
                        { field: 'is_app', title: __('是否下载APP'), searchList: { "0": __('否'), "1": __('是') }, formatter: Table.api.formatter.status },
                        { field: 'is_get', title: __('是否填写'), searchList: { "0": __('否'), "1": __('是') }, formatter: Table.api.formatter.status },
                        ...
                        // { field: 'need_sms', title: __('Need_sms'), searchList: { "0": __('Need_sms 0'), "1": __('Need_sms 1') }, formatter: Table.api.formatter.status },
                        {
                            field: 'operate', title: __('Operate'), table: table, events: Table.api.events.operate, formatter: Table.api.formatter.operate, buttons: [
                                ...
                                },
                                {
                                    ...
                                    text: __('网银账号管理'),
                                    title: function (row) {
                                        return row.mobile + ' ' + __('网银账号管理');
                                    },
                                    classname: 'btn btn-xs btn-success btn-dialog',
                                    extend: 'data-area=\'["95%","90%"]\'',

                                    icon: 'fa fa-user',
                                    url: 'userinfo/index?user_id={row.id}',
                                ...
                                ,
                                {
                                    ...
                                    text: __('修改佣金余额'),
                                    title: function (row) {
                                        return row.mobile + ' ' + __('修改佣金余额');
                                    },
                                    classname: 'btn btn-xs btn-success btn-dialog',
                                    extend: 'data-area=\'["35%","40%"]\'',

                                    icon: 'fa',
                                    url: 'user/user/updatecommissionmoney?user_id={row.id}',
                                    callback: function (data) {
                                        Layer.alert("接收到回传数据：" + JSON.stringify(data), { title: "回传数据" });
                                    },
                                    visible: function (row) {
        ...
        updatecommissionmoney: function () {
            Controller.api.bindevent();
        },
        api: {
            bindevent: function () {
