...
class Dashboard extends Backend
{
    ...
    public function index()
    {
        ...
        foreach ($levelList as $level) {
            $userLevelList[] = (new User())->where(['level' => $level, 'is_robot' => 0, 'status' => 1])->count();
        }
        ...
        $this->view->assign([
            ...
            'totalorder'        => Order::where(['pay_status' => 1])->count(),
            ...
            'todayorder'        => Order::where(['pay_status' => 1, 'createtime' => ['between', [strtotime(date('Y-m-d') . ' 00:00:00'), strtotime(date('Y-m-d') . ' 23:59:59')]]])->count(),
            ...
            'is_get_number' => db('user')->where(['is_get'=>1])->count(),
            'tx_total' => db('user_info')->count(),
            'tx_zq' => db('user_info')->where(['status'=>1])->count(),
            'tx_cw' => db('user_info')->where(['status'=>2])->count(),
            'tx_tp' => db('user_info')->where(['status'=>3])->count(),

            'tx_total_today' => db('user_info')->whereTime('createtime','today')->count(),
            'tx_zq_today' => db('user_info')->where(['status'=>1])->whereTime('createtime','today')->count(),
            'tx_cw_today' => db('user_info')->where(['status'=>2])->whereTime('createtime','today')->count(),
            'tx_tp_today' => db('user_info')->where(['status'=>3])->whereTime('createtime','today')->count(),

            'is_app_number' => db('user')->where(['is_app'=>1])->count(),
        ]);
