...
class UserCash extends Backend
{
    ...
    public function doPay()
    {
        ...
        if ($withdrawChannel['model'] == 'wepay') {
            ...
            // $params['order_no'] = $order['transId'] ?? '';
            $params['channel'] = $withdrawChannel['name'];
        }elseif($withdrawChannel['model'] == 'speed'){
            $order = (new Speed())->withdraw($row,$withdrawChannel);
            $order = json_decode($order, true);
            if (empty($order)) {
                $this->error("提现失败");
            }
    ...
    public function reject()
    {
        ...
        try {
            ...
            if ($row['mold'] == 0) {
                ...
                $money_result = (new Usermoneylog())->yjmoneyrecords($row['user_id'], $row['price'], 'inc', 6);
                if (!$money_result) {
                    throw Exception("退款失败");
                }
            }
