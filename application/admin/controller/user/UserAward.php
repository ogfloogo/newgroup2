...
class UserAward extends Backend
{
    ...
    protected function getUserAwardInfo($user_id)
    {
        ...
        $row['reward_amount'] = (new UserAwardModel())->where(['user_id' => $user_id, 'status' => ['GT', 0]])->sum('moneys');
        $row['reward_amount_received'] = (new UserAwardModel())->where(['user_id' => $user_id, 'status' => ['GT', 0]])->count();
        $row['reward_amount_unreceived'] = (new UserAwardModel())->where(['user_id' => $user_id, 'status' => 2])->count();
