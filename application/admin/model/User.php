...
class User extends Model
{
    ...
    protected static function init()
    {
        ...
        self::beforeUpdate(function ($row) {
            ...
            if (isset($changedata['commission']) && (function_exists('bccomp') ? bccomp($changedata['commission'], $origin['commission'], 2) !== 0 : (float)$changedata['commission'] !== (float)$origin['commission'])) {
                if (bccomp($changedata['commission'], $origin['commission'], 2) == 1) {
                    $mold = 'inc';
                } else {
                    $mold = 'dec';
                }
                if (isset($_REQUEST['remark'])) {
                    $remark =  $_REQUEST['remark'] ?? "管理员变更佣金余额";
                } else {
                    $remark =  "管理员变更佣金余额";
                }
                $type = 10; //变动类型:1=充值,2=提现,3=邀请奖励,4=佣金收入,5=团购下单,6=拒绝提现,7=团购奖励,8=团长奖励,9=新用户注册奖励,10=管理员操作
                $obj = (new UserUserMoneyLog());
                $obj->setTableName($row['id']);
                $obj->create(['user_id' => $row['id'], 'money' => abs(bcsub($changedata['commission'], $origin['commission'], 2)), 'before' => $origin['commission'], 'after' => $changedata['commission'], 'remark' => $remark, 'mold' => $mold, 'type' => $type, 'kind'=> 1]);
            }
        ...
        self::afterUpdate(function ($row) {
            ...
            if (isset($changedata['money']) && (function_exists('bccomp') ? bccomp($changedata['money'], $origin['money'], 2) !== 0 : (float)$changedata['money'] !== (float)$origin['money'])) {
                ...
                $cache = $redis->handler()->get("newgroup:token:" . $userinfo_new['token']);
                if ($cache) {
                    $redis->handler()->set("newgroup:token:" . $userinfo_new['token'], json_encode($userinfo_new->toArray()), 60 * 60 * 24);
                }
            }
            if (isset($changedata['sid']) && $changedata['sid'] != $origin['sid']) {
                (new UserTeam())->where(['team' => $row['id']])->delete();
                $data = [];
                $data['user_id'] = $changedata['sid'];
                $data['team'] = $row['id'];
                $data['level'] = 1;
                $data['createtime'] = time();
                (new UserTeam())->insert($data);
                // $userTeamList = (new UserTeam())->where(['user_id' => $row['id'], 'level' => ['gt', 0]])->order('level ASC')->select();
            }
            if (isset($changedata['commission']) && (function_exists('bccomp') ? bccomp($changedata['commission'], $origin['commission'], 2) !== 0 : (float)$changedata['commission'] !== (float)$origin['commission'])) {
                //刷新用户信息
                $userinfo_new = (new User())->where('id', $row['id'])->find();
