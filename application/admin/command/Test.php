...
class Test extends Command
{
    ...
    protected function firstRecharge()
    {
        $a = 500;
        $first_recharge_user_ids = [];
        // $user_ids = (new UserRecharge())->where(['price' => ['between', [100, 200]], 'status' => 1])->distinct(true)->column('user_id');
        $user_ids = (new UserRecharge())->where(['price' => ['egt', $a], 'status' => 1])->distinct(true)->column('user_id');
        $list = (new UserRecharge())->where(['user_id' => ['IN', $user_ids], 'status' => 1])->field('user_id,price,status')->order('paytime ASC')->select();
        foreach ($list as $item) {
            // if ($item['price'] <= 200 && $item['price'] >= 100) {
            if ($item['price'] >= $a) {
                if (!in_array($item['user_id'], $first_recharge_user_ids)) {
                    $first_recharge_user_ids[] = $item['user_id'];
                }
            }
        }
        print_r($first_recharge_user_ids);

        $total_user_ids = [];
        foreach ($first_recharge_user_ids as $user_id) {
            $amount = (new UserRecharge())->where(['user_id' => $user_id, 'status' => 1])->sum('price');
            if ($amount >= 1000) {
                if (!in_array($user_id, $total_user_ids)) {
                    $total_user_ids[] = $user_id;
                }
            }
        }
        print_r($total_user_ids);

        $data['first_recharge_num'] = count($first_recharge_user_ids);
        $data['first_recharge_amount'] = count($total_user_ids);
        print_r($data);
        return $data;
    }
