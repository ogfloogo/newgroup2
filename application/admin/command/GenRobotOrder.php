...
class GenRobotOrder extends Command
{
    ...
    //     $categoryInfo = $this->redisInstance->Hgetall("newgroup:category:" . $robotInfo['level']);
    ...
    //             $levelInfo = $this->redisInstance->hMget("newgroup:level:" . $userLevel['level'], ['commission_fee_level_1']);
    ...
    //             if ($userLevel && $userLevel['level'] >= $robotInfo['level'] && !$userLevel['is_robot']) {
    //                 $levelInfo = $this->redisInstance->hMget("newgroup:level:" . $userLevel['level'], ['commission_fee_level_2']);
    ...
    //                 if ($userLevel && $userLevel['level'] >= $robotInfo['level'] && !$userLevel['is_robot']) {
    //                     $levelInfo = $this->redisInstance->hMget("newgroup:level:" . $userLevel['level'], ['commission_fee_level_3']);
    ...
    //     //余额变动
    //     $isok = (new ModelUsermoneylog())->yjmoneyrecords($to_id, $commissions, 'inc', 4, "来源RID:" . $from_id);
    //     if (!$isok) {
    //         return false;
    //     }
    //     switch ($level) {
