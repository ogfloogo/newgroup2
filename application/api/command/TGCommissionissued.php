...
class TGCommissionissued extends Command
{
    ...
    {
        // 指令配置
        $this->setName('TGCommissionissued')
            ->setDescription('佣金发放');
    }
    ...
    {
        // 指令输出
        $output->writeln('TGCommissionissued');
        $ws = new \Swoole\WebSocket\Server('0.0.0.0', 9602);
        //守护进程模式
        $ws->set([
            'daemonize' => true,
            'worker_num' => 1,
        ...
        $ws->on('WorkerStart', function ($ws, $worker_id) {
            ...
            \Swoole\Timer::tick(3000, function () use ($redis, $commission, $usermoneylog, $usertotal, $order) {
                ...
                for ($i = 0; $i < 100; $i++) {
                    Db::startTrans();
                    try {
                        $value = $redis->handler()->lpop('newgroup:commissionlist');
                        if ($value) {
                            $list = explode("-", $value);
                            $to_id = $list[0]; //受益ID
                            $from_id = $list[1]; //来源ID
                            ...
                            //余额变动
                            $isok = $usermoneylog->yjmoneyrecords($to_id, $commissions, 'inc', 4, "来源ID:" . $from_id);
                            if ($isok == false) {
                                //失败插入
