...
class Controller extends \think\Controller
{
    ...
    public function _initialize()
    {
        ...
        $this->is_app = $this->request->header('isapp',0);
    ...
    public function verifyUser()
    {
        ...
        $tokens = $redis->handler()->get("newgroup:token:" . $this->token);
    ...
    public function getCacheUser()
    {
        ...
        $redis = new Redis();
        $redis->handler()->select(1);
        $user_info = $redis->handler()->get("newgroup:token:" . $this->token);
        if (!$user_info) {
            return false;
        }
    ...
    public static function getCacheUser2($token = '')
    {
        ...
        $redis = new Redis();
        $redis->handler()->select(1);
        $user_info = $redis->handler()->get("newgroup:token:" . $token);
        if (!$user_info) {
            return false;
        }
        return json_decode($user_info, true);
    ...
    public function updateCacheUser()
    {
        ...
        $redis->handler()->set("newgroup:token:" . $this->token, json_encode($userinfo), 60 * 60 * 24);
    }
