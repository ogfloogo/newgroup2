...
class Reward extends Controller
{
    ...
    public function signin(){
        ...
        $money = config('site.normal_signin');
    ...
    public function rewardPage(){
        ...
        foreach ($day_reward as &$value){
            $day_task = (new Usertask())->where(['user_id'=>$userinfo['id'],'category'=>2,'type'=>$value['type']])->find();
            ...
            if($value['type'] == 1){
                $value['path'] = "/pages/bank/bank";
            }elseif($value['type'] == 3){
                $value['path'] = "/pages/pay/pay";
            }
            elseif($value['type'] == 4){
     ...
     * @throws \think\exception\DbException
     */
    public function dayRewardReceive($user_id){
        $type = $this->request->post("type");
        if (!$type) {
            $this->error(__('parameter error'));
        }
        $day_task = (new Usertask())->where(['user_id'=>$user_id,'category'=>2,'type'=>$type,'is_condition'=>1])->whereTime('createtime','today')->find();
        //检查是否符合领取条件
        if($day_task){
