...
class Earnmoney extends Controller
{
    ...
    public function board()
    {
        ...
        $level = $redis->handler()->hMget('newgroup:level:' . $userinfo['level'], ['open_group_num']);
        //当日总开团次数
        ...
        if ($next_level_info) {
            //下一个等级预计收入
            $list['next_income'] = (new Userteam())->Imtoget($userinfo['level'] + 1);
            //下一个等级需要的金额
            $list['next_money'] = $next_level_info['become_balance'];
        }
    ...
    {
        $redis = new Redis();
        $highlist = $redis->handler()->ZRANGEBYSCORE('newgroup:high:set:0', '-inf', '+inf', ['withscores' => true]);
        $list = [];
        foreach ($highlist as $k => $v) {
            $list[] = $reward = $redis->handler()->hMget("newgroup:high:" . intval($k), ['name', 'team_income', 'reward_invite', 'level', 'become_balance']);
        }
        foreach ($list as $k => $v) {
