...
class Sms extends Controller
{
    ...
    public function send()
    {
        ...
        $last = $redis->handler()->get("newgroup:register:".$mobile);
        if ($last) {
        ...
        $last = $redis->handler()->set("newgroup:register:".$mobile,$code,600);
    ...
    public function authentication(){
        ...
        $redis->handler()->select(1);
        $last = $redis->handler()->get("newgroup:authentication:".$mobile);
        if ($last) {
            $this->error(__('Sending frequent'));
        }
        $code = Random::numeric(6);
        ...
        $last = $redis->handler()->set("newgroup:authentication:".$mobile,$code,600);
        $this->success(__('send successfully'));
    }
    ...
    public function sendreset()
    {
        ...
        $redis->handler()->select(1);
        $last = $redis->handler()->get("newgroup:resetpassword:".$mobile);
        if ($last) {
            $this->error(__('Sending frequent'));
        }
        $code = Random::numeric(6);
        ...
        $last = $redis->handler()->set("newgroup:resetpassword:".$mobile,$code,600);
        $this->success(__('send successfully'));
    }
    ...
    public function sendwithdraw()
    {
        ...
        $redis->handler()->select(1);
        $last = $redis->handler()->get("newgroup:resetwithdraw:".$mobile);
        if ($last) {
            $this->error(__('Sending frequent'));
        }
        $code = Random::numeric(6);
        ...
        $last = $redis->handler()->set("newgroup:resetwithdraw:".$mobile,$code,600);
        $this->success(__('send successfully'));
    }

   protected function sendCodeV3($phone,$code){
        $text ="[MayGroup] Otp anda ialah  ".$code.", sah selepas 10 minit. Jangan beritahu orang lain otp.";
    ...
    protected function sendCodeV($phone,$code){
        $text ="[WeGroup] You are applying for withdrawal verification code ".$code.",Please do not share this verification code with others to avoid losses.";
        $r = $this->sendV3($phone, $text);
        $r = json_decode($r, true);
        if($r['status'] != 0){
            return false;
    ...
    protected function sendV3($phone,$text){
        ...
        Log::mylog('header', $header, 'sms');
        Log::mylog('data', $data, 'sms');
