...
class Userwarehouse extends Controller
{
   ...
   /**
    ...
    public function exchangemoney(){
        ...
        if($status['type'] > 0){//非团购商品无法兑换
            $this->error(__('Inconvertible for cash'));
        }
    ...
    public function exchangemoneyall(){
        $this->verifyUser();
        $status = (new ModelUserwarehouse())->where(['user_id'=>$this->uid,'status'=>0,'type'=>0])->select();
        if(!$status){
            $this->error(__('operation failure'));
        }
        $userinfo = $this->userInfo;
        $return = (new ModelUserwarehouse())->exchangemoneyall($userinfo,$status);
        if(!$return){
            $this->error(__('operation failure'));
        }
        $this->success(__('operate successfully'));
