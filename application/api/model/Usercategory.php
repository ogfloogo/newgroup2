...
class Usercategory extends Model
{
    ...
    public function statistics($post, $userinfo, $id, $good_info)
    {
        $usermerchandise = new Usermerchandise();
        $usermerchandise->addlog($userinfo['id'], $good_info['id']);
        //必获商品所需次数
        $this->redisInstance = (new Redis())->handler();
        //该商品区已购买次数
        $num = $usermerchandise->where('user_id', $userinfo['id'])->where('category_id', $good_info['id'])->find();

        //是否被选中
        $is_win = $this->checkIsWin($userinfo['id'], $good_info);
        if ($is_win) {
            //如果已在今日中奖集合中，修改为未选中
            if ($this->isAddedWinPool($userinfo['id'], $good_info)) {
                $is_win = false;
            } else {
                //先加入中奖集合，失败也标记为未选中
                $is_added = $this->addWinPool($userinfo['id'], $good_info);
                if (!$is_added) {
                    $is_win = false;
                }
            }
        ...
        if ($is_win) {
            //开奖
            (new Userwarehouse())->drawwinning($post, $userinfo, $id, $good_info);
            ...
            $usermerchandise->where(['user_id' => $userinfo['id'], 'category_id' => $good_info['id']])->update($num_data);
            // if ($post['type'] == 1) {
            //     $group_head_reward = Config::get('site.group_head_reward');
            //     $bouns = (bcmul($good_info['reward'], intval($group_head_reward), 2)) / 100;
            //     //团长奖励
            ...
            //     //佣金发放
            //     $bouns = bcsub($good_info['buyback'], $good_info['price'], 2);
            ...
            //未中奖奖励
            // if ($post['type'] == 1) { //我要开团
            //     $tz = (new Usermoneylog())->opengrouprewards($userinfo['id'], $good_info, $id);
            //     if (!$tz) {
            //         return false;
            //     }
            // } else { //一键开团
            //     $tg = (new Usermoneylog())->akeytoopen($userinfo['id'], $good_info, $id);
            //     if (!$tg) {
            //         return false;
            //     }
            // }
            ...
            //未中奖，新增用户该商品下单次数
            $usermerchandise->where('user_id', $userinfo['id'])->where('category_id', $post['good_id'])->setInc('num', 1);
            //未中奖，新增用户当天下单次数
            $this->where('user_id', $userinfo['id'])->where('date', date('Y-m-d', time()))->setInc('num', 1);
            //更新订单
            (new Order())->where('id', $id)->update(["earnings" => $good_info['reward']]);
            //下单后检查是否可以进池
            $this->checkPoolIn($userinfo['id'], $good_info);
            return ['code' => 0];
        }
    }
    ...
    {
        return (new Goods())->cache_prefix . 'pool:' . $id;
    }
    ...
    public function getWinPoolKey($id, $date = '')
    {
        ...
        return (new Goods())->cache_prefix . 'pool:win:' . $id . ':' . $date;
    }

    protected function checkIsWin($user_id, $good_info)
    {
        Log::mylog('checkIsWin: good_info ', $good_info, 'win');
        Log::mylog('checkIsWin: user_id ', $user_id, 'win');
        // return $num && (($num['num'] + 1) >= intval($good_info['win_must_num']));
        $id = $good_info['id'];
        ...
        //如果今日额度没配不选
        if (!$good_info['daily_win_limit']) {
            return false;
        }
        Log::mylog('checkIsWin: sCard ', $this->redisInstance->sCard($win_pool_key) . '===' . $good_info['daily_win_limit'] . '---' . $this->redisInstance->sIsMember($win_pool_key, $user_id), 'win');

        //如果今日额度已到不选
        if ($this->redisInstance->sCard($win_pool_key) >= $good_info['daily_win_limit']) {
            return false;
        }
        ...
        $win_user_ids = $this->redisInstance->sRandMember($pool_key, $good_info['daily_win_limit']);
        //随机选择
    ...
    public function checkPoolIn($user_id, $good_info)
    {
        Log::mylog('checkPoolIn: good_info ', $good_info, 'win');
        Log::mylog('checkPoolIn: user_id ', $user_id, 'win');
        //今日已中不进池
        $id = $good_info['id'];
        $pool_key = $this->getPoolKey($id);

        if ($this->isAddedWinPool($user_id, $good_info)) {
        ...
        $userItem = (new Usermerchandise())->where(['user_id' => $user_id, 'category_id' => $good_info['id']])->find();
        Log::mylog('checkPoolIn: userItem ', $userItem, 'win');

        //入池条件
        $begin_time = $good_info['pool_in_num'];
        //冷冻条件
        $frozen_time = $good_info['win_must_num'];
    ...
    protected function isAddedWinPool($user_id, $good_info)
    {
        $id = $good_info['id'];
        $win_pool_key = $this->getWinPoolKey($id);
    ...
    protected function addWinPool($user_id, $good_info)
    {
    ...
    protected function addPool($user_id, $good_info)
    {
        $redisInstance = (new Redis())->handler();
        $id = $good_info['id'];
        $pool_key = $this->getPoolKey($id);
