...
class User extends Model
{
    ...
    public function login($post, $header = [])
    {
        ...
        try {
            //检测用户是否已注册
            $is_reg = $this->where('mobile', $post['mobile'])->find();
            if (!$is_reg) {
                return [
                    'code' => 0,
            ...
            $redis->handler()->set("newgroup:token:" . $token, json_encode($userinfo), 60 * 60 * 24 * 7);
            ...
            $experience_time = $redis->handler()->zScore("newgroup:sendlist", $userinfo['id']);
            if ($experience_time) {
                $userinfo['experience_time'] = $experience_time;
            } else {
                $userinfo['experience_time'] = 0;
            }
            //是否有等级升级记录
            $userinfo['level_log'] = $this->levellog($userinfo['id']);
            return [
                'code' => 1,
                'data' => $userinfo
            ];
    ...
    public function register($post, $agent_id = 0)
    {
        ...
        try {
            ...
            $insert = [
                ...
                'avatar' => "/mobile/static/image/home/avatar.png",
            ...
            if ($userinfo['sid'] != 0) {
                ...
                if (Config::get("site.invite_reward_cash")) {
                    (new Usermoneylog())->yjmoneyrecords($userinfo['sid'], Config::get('site.invite_reward_cash'), 'inc', 20, $user_id);
                }
            }
            //生成用户统计表
            ...
            $redis = new Redis();
            $redis->handler()->select(6);
            $redis->handler()->zAdd("newgroup:sendlist", time() + 60 * 60 * 24 * 3, $user_id);
            //提交
            Db::commit();
            //统计
            (new Shell())->addreport();
    ...
    public function registernew($post, $agent_id = 0)
    {
        ...
        try {
            ...
            //任务奖励
            (new Usertask())->oncetask($user_id,1);
            //新用户注册奖励
            // (new Usermoneylog())->moneyrecords($userinfo['id'], Config::get('site.user_register_reward'), 'inc', 9, $user_id);
            //邀请奖励
            if ($userinfo['sid'] != 0) {
                if (Config::get("site.invite_reward")) {
                    (new Usermoneylog())->yjmoneyrecords($userinfo['sid'], Config::get('site.invite_reward'), 'inc', 20, $user_id);
                    $this->inviterewards($userinfo);
                }
            }
            //生成用户统计表
    ...
    public function logout($token)
    {
        $redis = new Redis();
        $redis->handler()->select(1);
        $redis->handler()->del('newgroup:token:' . $token);
    }
    ...
    public function refresh($user_id)
    {
        ...
        if ($cache) {
            $redis->handler()->set("newgroup:token:" . $userinfo_news['token'], json_encode($userinfo_news), 60 * 60 * 24);
        }
    }
