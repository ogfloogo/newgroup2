...
class Userwarehouse extends Model
{
    ...
    public function list($post, $user_id)
    {
        ...
        $list = $this
            ...
            ->where($where)
            ->field('a.id,a.good_id,a.type,a.order_id,a.createtime,c.is_default,c.name,c.mobile,c.postcode,c.address,c.province,city,county,village,b.status')
        ...
        if ($list) {
            foreach ($list as $key => $value) {
                ...
                if ($goodsinfo) {
                    ...
                    $list[$key]['price'] = (db('order')->where('id', $value['order_id'])->find())['amount'] ?? 0;
                    //回购金额
                    $order_detail = db('order')->where('id', $value['order_id'])->find();
                    $list[$key]['buyback'] = $order_detail['buyback'] ?? 0;
                }
            }
        }
        return $list;
     ...
     * 中奖，入库
     */
    public function drawwinning($post, $userinfo, $id, $good_info)
    {
        $insert = [
            'user_id' => $userinfo['id'],
            'good_id' => $post['good_id'],
            'order_id' => $id,
            'buyback' => $good_info['buyback'],
     ...
     * 秒杀，新人下单入库
     */
    public function drawwinnings($post, $userinfo, $id, $good_info,$type)
    {
        $insert = [
            'user_id' => $userinfo['id'],
            'good_id' => $post['good_id'],
     ...
     * 兑换全部现金 All
     */
    public function exchangemoneyall($userinfo,$list)
    {
        foreach ($list as $key => $value) {
