...
class Popups extends Model
{
    ...
    public function getList($is_login = 0)
    {
        ...
        $cache_key = "newgroup:popup:set:" . $is_login;

        $redis = new Redis();
        $list = $redis->handler()->zRange($cache_key, 0, -1);
        $res = [];
        foreach ($list as $k => $id) {
            $info = $redis->handler()->hMget('newgroup:popup:' . intval($id), ['name', 'is_login', 'image', 'url']);
            //  $info = $redis->handler()->hGetAll('newgroup:popup:' . intval($id));
            //  unset($info['id']);
