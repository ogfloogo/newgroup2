...
class Userteam extends Model
{
    ...
    public function myteamtotal($userid, $userinfo)
    {
        ...
        $rate_list = [Config::get("site.first"),Config::get("site.second"),Config::get("site.third")];
        for ($i = 0; $i < 3; $i++) {
            $time = Time::today();
            ...
            $list[$i]['commission_rate'] = $rate_list[$i];
        ...
        if($list){
            ...
            foreach($list as $key=>$value){
                ...
                $levelinfo = $redis->handler()->hMget("newgroup:level:" . $value['level'], [$commission_fee_level]);
                                $levelid = $redis->handler()->hMget("newgroup:level:" . $value['level'], ['id']);
